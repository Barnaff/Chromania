...
public class GameSparksTestUI : MonoBehaviour
{
	...
	void OnGUI ()
	{
        ...
        if (GUILayout.Button("post Leaderboard", GUILayout.Width(itemWidth), GUILayout.Height(itemHeight)))
        {
            ...
            colorsList.Add(0);
            colorsList.Add(0);
            dataDictionary.Add("colors", colorsList);


            GSData data = new GSData(dataDictionary);

            new LogEventRequest_POST_CS_SCORE().Set_SCORE(20).Set_COLOR_1(2).Send((response) =>
            {
                if (response.HasErrors)
                {
                    Debug.Log("error posting leaderboard");
                }
                else
                {
                    Debug.Log("sucsess");
            ...
            new LeaderboardDataRequest ().SetLeaderboardShortCode ("CS").SetEntryCount (10) .Send ((leadResponse) => {
