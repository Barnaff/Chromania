...
namespace MovementEffects
{
    public class Timing : MonoBehaviour
    {
        ...
        public bool AdditionalDebugInfo = false;
        ...
        private void Update()
        {
            if(_lastSlowUpdateTime + TimeBetweenSlowUpdateCalls < Time.realtimeSinceStartup && _nextSlowUpdateProcessSlot > 0)
            {
                ...
                for (coindex.i = 0; coindex.i < _nextSlowUpdateProcessSlot; coindex.i++)
                {
                    ...
                    {
                        Profiler.BeginSample(AdditionalDebugInfo ? ("Processing Coroutine (Slow Update)" +
                            (_processTags.ContainsKey(coindex) ? ", tag " + _processTags[coindex] : ", no tag")) : "Processing Coroutine (Slow Update)");
            ...
            if (_nextUpdateProcessSlot > 0)
            {
                ...
                for (coindex.i = 0; coindex.i < _nextUpdateProcessSlot; coindex.i++)
                {
                    ...
                    {
                        Profiler.BeginSample(AdditionalDebugInfo ? ("Processing Coroutine" +
