...
public class TouchManager : MonoBehaviour {
    ...
    /// The touch prefab.
    /// </summary>
    public GameObject _touchPrefab;
		...
		else
		{
			if (Input.GetMouseButtonDown(0))
			{
				if (!_touches.Contains(0))
				{
					Vector3 mousePosition = Camera.main.ScreenToWorldPoint(Input.mousePosition);
					mousePosition.z = 0;
					GameObject touchController = Lean.LeanPool.Spawn(_touchPrefab, mousePosition, Quaternion.identity) as GameObject;
	...
	private void CreateTouch(Touch touch)
	{
		...
		position.z = 0.0f;
		GameObject touchController = Lean.LeanPool.Spawn(_touchPrefab, position, Quaternion.identity) as GameObject;
        touchController.GetComponent<TouchController>().Init();
