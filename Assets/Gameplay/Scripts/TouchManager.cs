...
public class TouchManager : MonoBehaviour {
		...
		else
		{
			if (Input.GetMouseButtonDown(0))
			{
				if (!_touches.Contains(0))
				{
					Vector3 mousePosition = Camera.main.ScreenToWorldPoint(Input.mousePosition);
					mousePosition.z = 0;
					GameObject touchController = Instantiate(TouchPrefab, mousePosition, Quaternion.identity) as GameObject;
	...
	private void CreateTouch(Touch touch)
	{
		...
		position.z = 0.0f;
		GameObject touchController = Instantiate(TouchPrefab, position , Quaternion.identity) as GameObject;
